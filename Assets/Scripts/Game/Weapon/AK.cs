...
namespace QFramework.Gungeon
{
	public partial class AK : QFramework.Gungeon.Gun
	{
        ...
        public override BulletBag bulletBag { get; set; } = new BulletBag(90);
        ...
        public void Shoot(Vector2 direction)
        {
            if (clip.CanShoot)
            {
                var angle = direction.ToAngle() + Random.Range(-UnstableRate, UnstableRate) * 30 * 2.5f;
                ...
                CameraController.Shake.Trigger(3, 0.06f);

                BackForce.Shoot(0.08f, 2);

                clip.UseBullet();

                BulletFactory.GenBulletShell(direction,BulletFactory.Default.AKShell);
            }
